...
foreach ($alert in $openAlerts) {
    ...
    # Recalculate the health state of the monitoring object
    if ($monitoringObject) {
        Write-Host "Recalculating health for: $($monitoringObject.DisplayName)"
        $monitoringObject.RecalculateMonitoringState()
    }
    else {
